      ...
      <div className="max-w-[140rem] mx-auto px-3 sm:px-5">
        ...
        <div className="flex items-center gap-2 h-14">
          ...
          <Link to="/" className="flex items-center gap-2 flex-shrink-0 group">
            ...
            />
            <span className="font-bold text-sm sm:text-base tracking-tight hidden sm:block text-white" style={{ textShadow: '0 0 12px rgba(180,255,0,0.3)' }}>
              ETHIODO
            </span>
          </Link>

          {/* Nav links — scrollable on small screens */}
          <div className="flex items-center gap-0.5 sm:gap-1 mx-1 sm:mx-2 flex-shrink overflow-x-auto scrollbar-none">
            {navLinks.map(link => (
              <Link
                key={link.path}
                to={link.path}
                className="px-2 sm:px-3 py-1.5 rounded-full text-[11px] sm:text-xs font-medium transition-colors whitespace-nowrap flex-shrink-0"
                style={{
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 flex-shrink-0">
            ...
            {userLoaded && (user ? (
              <div className="flex items-center gap-1 sm:gap-1.5">
                ...
                <Link to="/profile" className="sm:hidden flex items-center justify-center w-8 h-8 rounded-full transition-all" style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)' }}>
        ...
        {/* ── Row 2: full-width search bar (shown when onSearchChange provided) ── */}
        {onSearchChange && (
