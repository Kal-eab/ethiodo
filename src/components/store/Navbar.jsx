...
export default function Navbar({ onSearchChange = null, searchValue = '', category = 'all', onCategoryChange = null }) {
  ...
  useEffect(() => {
    ...
    const handler = (e) => { if (!localStorage.getItem('theme')) setDarkMode(e.matches); };
  ...
  // Keep --navbar-height CSS variable in sync — only measures the <nav>, not the dropdown
  useEffect(() => {
  ...
  return (
    <>
        ...
        <div className="max-w-[140rem] mx-auto px-3 sm:px-5">
          ...
          <div className="flex items-center gap-2 h-14">
            ...
            <Link to="/" className="flex items-center gap-2 flex-shrink-0 group">
              ...
              />
              <span className="font-bold text-sm sm:text-base tracking-tight text-white" style={{ textShadow: '0 0 12px rgba(180,255,0,0.3)' }}>
                ETHIODO
              </span>
            </Link>
      ...
      {/* Mobile dropdown — outside <nav> so it doesn't affect navbar height / category bar position */}
      {mobileOpen && (
        <div
          className="md:hidden fixed left-0 right-0 z-40 px-4 pb-4 pt-2"
          style={{ top: 'var(--navbar-height, 56px)', background: '#111', borderBottom: '1px solid rgba(180,255,0,0.1)' }}
        >
