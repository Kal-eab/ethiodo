...
import {
  ...
  ArrowRight, ArrowLeft, CheckCircle, Mail, Lock, Eye, EyeOff
} from 'lucide-react';
...
export default function Register() {
  ...
  const handleNext = () => {
    if (step === 0) {
      if (!alreadyAuthed) {
        // Not logged in — redirect to Base44 signup then come back
        if (!validateStep0()) return;
        // Save progress to session storage to restore after redirect
  ...
  const handleSubmit = async () => {
    ...
    toast.success('Profile complete! Welcome to Ethiodo 🎉');
    setSaving(false);
    navigate('/');
  };
    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center px-4 py-10"
      ...
      <div className="w-full max-w-md bg-card border border-border shadow-2xl overflow-hidden">
        ...
        <div className="px-6 pt-6 pb-4 border-b border-border"
          ...
          <p className="text-muted-foreground text-xs font-mono">Join Ethiodo — Ethiopia's premier online store</p>
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          {step === 0 && (
            <>
              ...
              {!alreadyAuthed && (
                <div className="border border-border/50 bg-secondary/30 p-3 space-y-1">
                  <p className="font-mono text-[11px] text-muted-foreground">
                    <span className="text-primary font-bold">Next:</span> You'll be taken to create your email & password, then come back to complete your address.
                  </p>
                </div>
              )}
            </>
            ...
            <>
              ...
              <div>
                ...
                <Input
                  ...
                  className="bg-secondary border-border h-11"
                  placeholder="e.g. near Edna Mall, behind Blue Building, Sefer name"
                />
                <p className="font-mono text-[10px] text-muted-foreground mt-1">
                  Name of your sefer, landmark, or building to help delivery find you
                </p>
              </div>
            </>
          )}
        ...
        <div className="px-6 pb-6 pt-1 flex gap-3">
          ...
          {step < STEPS.length - 1 ? (
            ...
            >
              {alreadyAuthed ? 'Next' : 'Continue to Sign Up'} <ArrowRight className="w-4 h-4" />
            </button>
          ) : (
            <button
